...
*/
	...
	default:
		WBUG("facio " TLONG_FMT " in JvmPort", pius->ordo);
		return facioJava(pius);
	}
	return true;
}
...
extern "C" TEXTUS_AMOR_EXPORT void textus_get_version_1(char *scm_id, char *time_str, char *ver_no, int len) 
{
	...
	if ( aver )
	{
		...
		mlen = (short)strlen(aver->cls_name);
		memcpy(scm_id, aver->cls_name, mlen > len ? len : mlen);

		GET_SCM_VERSION_INFO(aver->mod_time, time_str)
		mlen = (short)strlen(aver->build_no);
		GET_SCM_VERSION_INFO(aver->build_no, ver_no)
		delete aver;
	}
}

static TEXTUS_LONG ba2buf(JNIEnv * env, jbyteArray args, unsigned char*buf, int buf_len)
{
        jsize len, r_len;
...
static void *getPointer (JNIEnv *env,  jobject obj)
{
	...
	size_t psize = env->GetArrayLength(port);
	if ( psize !=  sizeof(pointer))
		return 0;
	ba2buf(env, port, reinterpret_cast<unsigned char*>(&pointer), sizeof(pointer));
...
static void setPointer (JNIEnv *env,  jobject obj, void *p)
{
	jclass o_cls = env->GetObjectClass(obj); 
	jfieldID port_fld = env->GetFieldID(o_cls, "portPtr", "[B");	
	jbyteArray port =  buf2ba(env, reinterpret_cast<unsigned char*>(&p) , sizeof(p));
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_grant (JNIEnv *env, jobject paco, jlong space)
{
	struct PacketObj *pcp = (struct PacketObj *) getPointer(env,paco);
	//printf("in jni pacdata %p\n", pcp);
	if ( pcp)
		pcp->grant(space);
	return;
}
...
JNIEXPORT jbyteArray JNICALL Java_textor_jvmport_PacketData_getfld (JNIEnv *env, jobject paco, jint no)
{
	...
	if ( pcp)
	{
		...
		if ( val && len > 0 )
			reta =  buf2ba(env, val, static_cast<int>(len));
	}
	return reta;
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__IB (JNIEnv *env, jobject paco, jint no, jbyte val)
{
	...
	if ( pcp)
	{
		pcp->input(no, reinterpret_cast<unsigned char*>(&val), sizeof(jbyte));
	}
	return;
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__II (JNIEnv *env, jobject paco, jint no , jint iVal)
{
	...
	if ( pcp)
	{
		pcp->input(no, reinterpret_cast<unsigned char*>(&iVal), sizeof(jint));
	}
	return;
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__IJ (JNIEnv *env, jobject paco, jint no, jlong jVal)
{
	...
	if ( pcp)
	{
		pcp->input(no, reinterpret_cast<unsigned char*>(&jVal), sizeof(jlong));
	}
	return;
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__ILjava_lang_Boolean_2 (JNIEnv *env, jobject paco, jint no, jobject bVal)
{
	...
	if ( pcp)
	{
		...
		pcp->input(no, reinterpret_cast<unsigned char*>(yes), sizeof(jboolean));
	}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__IZ (JNIEnv *env, jobject paco, jint no, jboolean blVal)
{
	...
	if ( pcp)
	{
		pcp->input(no, reinterpret_cast<unsigned char*>(&blVal), sizeof(jboolean));
	}
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__IS (JNIEnv *env, jobject paco, jint no, jshort sVal)
{
	...
	if ( pcp)
	{
		pcp->input(no, reinterpret_cast<unsigned char*>(&sVal), sizeof(jshort));
	}
	return;
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__IF (JNIEnv *env, jobject paco, jint no, jfloat fVal)
{
	...
	if ( pcp)
	{
		pcp->input(no, reinterpret_cast<unsigned char*>(&fVal), sizeof(jfloat));
	}
	return;
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__ID (JNIEnv *env, jobject paco, jint no, jdouble dVal)
{
	...
	if ( pcp)
	{
		pcp->input(no, reinterpret_cast<unsigned char*>(&dVal), sizeof(jdouble));
	}
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__ILjava_sql_Date_2 (JNIEnv *env, jobject paco, jint no, jobject dtVal)
{
	...
	if ( pcp)
	{
		timep = (jlong*)env->CallObjectMethod(dtVal, getVal_mid);
		pcp->grant(sizeof(jlong));
		pcp->input(no, reinterpret_cast<unsigned char*>(timep), sizeof(jlong));
	}
	env->DeleteLocalRef(d_cls);
	return;
}
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__ILjava_sql_Time_2 (JNIEnv *env, jobject paco, jint no, jobject tmVal)
{
	...
	if ( pcp)
	{
		timep = (jlong*)env->CallObjectMethod(tmVal, getVal_mid);
		pcp->grant(sizeof(jlong));
		pcp->input(no, reinterpret_cast<unsigned char*>(timep), sizeof(jlong));
	}
	env->DeleteLocalRef(d_cls);
...
JNIEXPORT void JNICALL Java_textor_jvmport_PacketData_input__ILjava_sql_Timestamp_2 (JNIEnv *env, jobject paco, jint no, jobject stmVal)
{
	...
	if ( pcp)
	{
		timep = (jlong*)env->CallObjectMethod(stmVal, getVal_mid);
		pcp->grant(sizeof(jlong));
		pcp->input(no, reinterpret_cast<unsigned char*>(timep), sizeof(jlong));
	}
	env->DeleteLocalRef(d_cls);
...
JNIEXPORT void JNICALL Java_textor_jvmport_TBuffer_alloc
  (JNIEnv *env , jobject tbo, jlong size)
{
...
JNIEXPORT jlong JNICALL Java_textor_jvmport_TBuffer_commit
  (JNIEnv *env, jobject tbo, jbyteArray ptr, jlong len)
{
	TBuffer *tbp = (TBuffer *)getPointer(env, tbo);
	if ( tbp ) 
...
JNIEXPORT void JNICALL Java_textor_jvmport_TBuffer_pour__Ltextor_jvmport_TBuffer_2Ltextor_jvmport_TBuffer_2
  (JNIEnv *env, jclass tb_cls, jobject tba, jobject tbb)
{
	TBuffer *tbpa = (TBuffer *)getPointer(env, tba);
	TBuffer *tbpb = (TBuffer *)getPointer(env, tbb);
...
JNIEXPORT void JNICALL Java_textor_jvmport_TBuffer_pour__Ltextor_jvmport_TBuffer_2Ltextor_jvmport_TBuffer_2J
  (JNIEnv *env, jclass tb_cls, jobject tba, jobject tbb, jlong len)
{
	TBuffer *tbpa = (TBuffer *)getPointer(env, tba);
	TBuffer *tbpb = (TBuffer *)getPointer(env, tbb);
	if ( tbpa && tbpb)
...
static void getPiusIndic (JNIEnv *env,  Amor::Pius &pius, jobject ps, jobject amor)
{
	...
	pius.ordo = (TEXTUS_ORDO)env->GetLongField(ps, ordo_fld);
	...
	case Notitia::SET_TINY_XML:
	{
		...
		ba2buf(env, fir, reinterpret_cast<unsigned char*>(&(((void**)pius.indic)[0])), sizeof(void *));
		ba2buf(env, sec, reinterpret_cast<unsigned char*>(&(((void**)pius.indic)[1])), sizeof(void *));
...
static void freePiusIndic (JNIEnv *env, Amor::Pius &pius, jobject ps) 
{
	...
	case Notitia::DMD_SET_ALARM:
	{
		...
		int *click = reinterpret_cast<int*>(indp[1]);
		delete[] click;
		delete[] indp;
	}
		break;
	...
	case Notitia::Get_WS_MsgType:
	{
		unsigned char *op = reinterpret_cast<unsigned char*>(pius.indic);
		delete op;
	}
		break;
...
bool JvmPort::pius2Java (Pius *pius, jmethodID fs_mid)
{
		...
		/*  *indic[0] = argc, *indic[1] = argv, 将此转为String[] */
	{
		...
		ps = (void**)pius->indic;
 		num = *(reinterpret_cast<int*>(ps[0]));
		argv = reinterpret_cast<char **>(ps[1]);
		...
		/* 这些要转一个java.lang.Integer */
	{
		...
		integer_val = getIntegerObj(jvmcfg->env,(int)(*(reinterpret_cast<unsigned char*> (pius->indic))));
		jvmcfg->env->SetObjectField(ps_obj, indic_fld, integer_val);
		if (  jvmcfg->env->CallBooleanMethod(owner_obj, fs_mid, ps_obj) ) fs_ret = true ; else fs_ret = false;
		jvmcfg->env->DeleteLocalRef(integer_val);
	}
		break;
...
#define OBJ_SET_STR(X,Y) \
	if ( dface->Y == 0 ) { \
		...
		len = (jsize)strlen((const char *)dface->Y);	\
		args =  env->NewByteArray(len);		\
...
#define OBJ_SET_STATIC_INT(X,Y) \
	...
	//OBJ_SET_INT("outSize", outSize); //use next a line
	env->SetLongField(face_obj, env->GetFieldID(face_cls, "outSize", "J"), dface->outSize);
	switch (dface->in )
	{
	case DBFace::FIRST:
...
#define OBJ_PARA_SET_INT(X,Y) \
	env->SetIntField(para_obj, env->GetFieldID(dbpara_cls, X, "I"), dface->paras[i].Y);

#define OBJ_PARA_SET_STR(X,Y) \
	if ( dface->paras[i].Y ) \
	{	\
	len = (jsize) strlen((const char *)dface->paras[i].Y);	\
	args =  env->NewByteArray(len);		\
