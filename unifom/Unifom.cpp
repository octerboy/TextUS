...
void Unifom::ignite(TiXmlElement *prop)
{
	...
	gCFG->hasPacket = false;
	if ( (comm_str = prop->Attribute("provide")) && strcasecmp(comm_str, "packet") == 0 )
		gCFG->hasPacket = true;

	gCFG->onlyMap = false;
	if ( (comm_str = prop->Attribute("mapOnly")) && strcasecmp(comm_str, "yes") == 0 )
		gCFG->onlyMap = true;
...
bool Unifom::facio( Amor::Pius *pius)
{
	...
	case Notitia::SET_UNIPAC:
		...
		if ( !gCFG->hasPacket )	/* ×Ô¼º²»´´½¨ÓÒ±ßPacketObj, ¼ÌÐøÏòÓÒ±ß´«µÝ  */
			aptus->facio(pius);
		break;
...
bool Unifom::sponte( Amor::Pius *pius)
{
	...
	case Notitia::SET_UNIPAC:
		...
		if ( gCFG->hasPacket )	/* ×Ô¼ºÓÐ, ²»½ÓÊÜÓÒ±ßµÄ */
			break;
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400 )
	tdatePtr = &tdate;
	gmtime_s(&tdate, &now.time );
#else
	tdatePtr = gmtime( &now.time );
#endif
	if (def.format )
	{
		char millstr[64];
