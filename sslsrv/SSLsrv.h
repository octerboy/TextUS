...
public:
	...
	void ssl_down(bool &);	//ssl关闭时, 向客户端通知
	bool handshake_ok;	/* false: 还需要SSL_accept()
...
private:
	...
	int clasp(bool &has);	//新的SSL会话握手, 返回0: 还在握手, 1: 握手成功, -1: 出现错误,应当终止(但这个函数不终止ssl)
	void novo();	//新的SSL会话
	bool matris;
};
#endif
